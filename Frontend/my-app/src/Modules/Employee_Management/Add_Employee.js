...
const Add_Employee = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'Performance_Evaluation' && (updatedValue < 1 || updatedValue > 10)) {
      toast.error('Please enter a value between 1 and 10.');
      return; // Prevent updating state if the value is invalid
    }

    // Update formData and calculate Total_Deduction_Amount
    setFormData((prevData) => {
      const newSalary = name === 'Salary' ? updatedValue : prevData.Salary;
      const newDeductionRate = name === 'Deduction_rate' ? updatedValue : prevData.Deduction_rate;

      const totalDeductionAmount = (newSalary * newDeductionRate) / 100;
 ...
 e.preventDefault();
    ...
    try {
      console.log(files)
      const response = await axios.post('http://apisanta.devcir.co/api/employees', formDataToSend, {
        headers: {
